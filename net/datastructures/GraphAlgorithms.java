...
public class GraphAlgorithms {
  ...
  public static <V,E> void DFS(Graph<V, E> g, Vertex<V> u, Set<Vertex<V>> known, 
    Map<Vertex<V>, Edge<E>> forest, String[] stations) {
    
    known.add(u);                              // u has been discovered
    for (Edge<E> e : g.outgoingEdges(u)) {     // for every outgoing edge from u
      if(!e.getElement().equals(90)) {
        Vertex<V> v = g.opposite(u, e);
        if (!known.contains(v)) {
          forest.put(v, e);                      // e is the tree edge that discovered v
          Integer tmp = (Integer)v.getElement();
          
          System.out.println("Station #" + v.getElement() + ": " + stations[tmp]);
          
          DFS(g, v, known, forest, stations);              // recursively explore from v
        }
      }
    }
  }
   ...
   */
  public static <V,E> Map<Vertex<V>,Edge<E>> DFSComplete(Graph<V,E> g, Vertex<V> start, String[] stations) {
    Set<Vertex<V>> known = new HashSet<>();
    Map<Vertex<V>,Edge<E>> forest = new ProbeHashMap<>();

      if (!known.contains(start))
        DFS(g, start, known, forest, stations);            // (re)start the DFS process at u
    return forest;
  }
  ...
  shortestPathLengths(Graph<V,Integer> g, Vertex<V> src, Vertex<V> fin) {
    ...
    while (!pq.isEmpty()) {
      ...
      if (u.getElement() == fin.getElement()) {
        return cloud;
      }
  ...
  verticesInShortestPath(Graph<V,Integer> g, Vertex<V> src, Vertex<V> fin) {

    java.util.List<V> verticesInOrder = new ArrayList<V>();

    V srcE = src.getElement();
    V finE = fin.getElement();
    int totalWeight = Integer.MAX_VALUE;

    java.util.Map<V, V> intermediateVs = new HashMap<V, V>();
    // d.get(v) is upper bound on distance from src to v
    Map<Vertex<V>, Integer> d = new ProbeHashMap<>();
    // map reachable v to its d value
    Map<Vertex<V>, Integer> cloud = new ProbeHashMap<>();
      ...
      for (Edge<Integer> e : g.outgoingEdges(u)) {
        ...
        // perform relaxation step on edge (u,v)
          ...
          if (d.get(u) + wgt < d.get(v)) {              // better path to v?
            ...
            intermediateVs.put(v.getElement(), u.getElement());

            // System.out.println("From " + u.getElement() + " to " + v.getElement() + " takes " + wgt + " seconds. Accumulated time is " + d.get(v) + " seconds.");
            pq.replaceKey(pqTokens.get(v), d.get(v));   // update the pq entry
          }
        }
      }
    }
    verticesInOrder.add(0, finE);
    V tmp = finE;
    do {
      tmp = intermediateVs.get(tmp);
      verticesInOrder.add(0, tmp);
    }
    while (tmp != srcE);

    return verticesInOrder;         // this only includes reachable vertices
  }
